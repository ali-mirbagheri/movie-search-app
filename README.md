...
- `VITE_API_URL`: Base URL for API requests (e.g., `https://api.themoviedb.org/3`). This is the current TMDB API endpoint, but it may change in the future, so check the official TMDB documentation if needed.
- `VITE_SECRET_KEY`: Secret key for encryption (e.g., `your-secret-key-123`). This should be a custom, secure key generated by the user for encryption purposes.
- `VITE_IMAGE_URL`: Base URL for image assets (e.g., `https://image.tmdb.org/t/p/w500`). This is the standard TMDB image endpoint, which is typically stable but may change in the future, so verify with TMDB documentation if issues arise.
- `VITE_API_KEY`: API key for TMDB (required for API access). To obtain a TMDB API key, sign up at [https://www.themoviedb.org/](https://www.themoviedb.org/) and generate an API key from your account settings.
...
1. Clone the repository:
   ...
   ```bash
   git clone https://github.com/ali-mirbagheri/movie-search-app
   cd movie-search-app
   ```
...
3. Run the container (optional: override env vars at runtime for security):
   ...
   docker run -p 3000:3000 \
     -e VITE_API_URL=https://api.themoviedb.org/3 \
...
├── shared/           # Reusable code (components, utils, theme, config, translations)
├── index.css         # Global CSS styles for the project
├── App.tsx           # Main app component handling routing and authentication logic
├── main.tsx          # Entry point, wrapping the app with React Toastify, React Query, and Material-UI
